...
const Collection = () => {
  ...
  return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t border-gray-700 bg-gray-900 text-gray-300'>

      {/* Filter Options */}
      <div className='min-w-60'>
        <p onClick={() => setShowFilter(!showFilter)} className='my-2 text-xl flex items-center cursor-pointer gap-2 text-gray-200'>FILTERS<img className={`h-3 sm:hidden ${showFilter ? ' rotate-90' : ''}`} src={assets.dropdown_icon} alt="" /></p>

        {/* Category Filter */}
        <div className={`border border-gray-600 pl-5 py-3 mt-6 ${showFilter ? '' : 'hidden'} sm:block`}>
          <p className='mb-3 text-sm font-medium text-gray-200'>CATEGORIES</p>
        ...
        <div className={`border border-gray-600 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
          <p className='mb-3 text-sm font-medium text-gray-200'>TYPE</p>
          <div className='flex flex-col gap-2 text-sm font-light text-gray-400'>
      ...
      <div className='flex-1'>
        ...
        <div className='flex justify-between text-base sm:text-2xl mb-4'>
          ...
          {/* Product Sort */}
          <select onChange={(e) => setSortType(e.target.value)} className='border-2 border-gray-600 text-sm px-2 bg-gray-800 text-gray-300' name="" id="">
